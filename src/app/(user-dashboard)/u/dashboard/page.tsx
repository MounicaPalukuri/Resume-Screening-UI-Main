...
import { Plus, Edit, Trash2, X, Check, Calendar, Bell, MessageSquare, Send, Sparkles, ShieldCheck, Activity } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const [chatMessages, setChatMessages] = useState<ChatMessage[]>([
    {
      id: 1,
      role: "assistant",
      text: "Hi! Ask me about your resume, mock test prep, or next steps.",
    },
  ]);
  const [chatInput, setChatInput] = useState("");
  const [analysisScore] = useState(78);
  const [analysisTakeaway] = useState(
    "Your resume is strong, but it can be improved by highlighting measurable results and keywords tailored to your target roles."
  );
  const [analysisRecommendations] = useState([
    "Add quantifiable achievements to your experience sections.",
    "Use role-specific keywords in your summary and skills.",
    "Showcase leadership, collaboration, and project outcomes.",
  ]);
  ...
  const [resumeScoreState, setResumeScoreState] = useState(78);
  const [candidateRankState, setCandidateRankState] = useState(92);
  ...
  useEffect(() => {
    ...
    const savedResume = localStorage.getItem("smartHireResumeUpload");
    const savedInterviews = localStorage.getItem("smartHireScheduledInterviews");
    const savedAnalysis = localStorage.getItem("smartHireResumeAnalysis");

    setTotalResumesUploaded(savedResume ? 1 : 0);

    if (savedInterviews) {
      const interviews = JSON.parse(savedInterviews);
      setInterviewsScheduledCount(Array.isArray(interviews) ? interviews.length : 0);
    }

    if (savedAnalysis) {
      try {
        const parsed = JSON.parse(savedAnalysis);
        if (parsed.score) setResumeScoreState(parsed.score);
        if (parsed.rank) setCandidateRankState(parsed.rank);
      } catch {
        // ignore invalid stored analysis
      }
    }
  }, []);
  ...
  const generateAutoReply = (message: string) => {
    ...
    if (lower.includes("resume")) {
      return "Your resume looks good, and I recommend adding more keywords from the job description to improve match rates.";
    }
    if (lower.includes("mock") || lower.includes("test")) {
      return "Focus on time management and review the weak areas in your latest mock test. Practice questions on core topics will help improve your score.";
    }
    if (lower.includes("interview") || lower.includes("schedule") || lower.includes("booking")) {
      return "Interview scheduling is easy: choose a candidate, select a date and time, and your meeting will be stored on the dashboard.";
    }
    if (lower.includes("skills")) {
      return "Skills detected: Java, Python, SQL, React. Focus on matching keywords and showcasing measurable achievements in your resume.";
    }
    if (lower.includes("dashboard") || lower.includes("progress")) {
      return "Your dashboard summarizes current progress clearly. Check the mock test and resume analysis widgets for immediate improvement areas.";
    }
    return "Please ask about resume screening, interview preparation, or skills.";
  };
    ...
    setTimeout(() => {
      setChatMessages((prev) => [
        ...prev,
        {
          id: Date.now() + 1,
          role: "assistant",
          text: generateAutoReply(trimmed),
        },
      ]);
    }, 450);
  };
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-white to-gray-50 dark:from-[#0f1a1a] dark:to-[#0a1414] px-6 py-6 transition-colors">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 mb-6">
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-[#111827]">
          <p className="text-sm text-gray-500 dark:text-gray-400">Total Resumes Uploaded</p>
          <p className="mt-3 text-3xl font-semibold text-gray-900 dark:text-white">{totalResumesUploaded}</p>
        </div>
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-[#111827]">
          <p className="text-sm text-gray-500 dark:text-gray-400">Interviews Scheduled</p>
          <p className="mt-3 text-3xl font-semibold text-gray-900 dark:text-white">{interviewsScheduledCount}</p>
        </div>
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-[#111827]">
          <p className="text-sm text-gray-500 dark:text-gray-400">Resume Score</p>
          <p className="mt-3 text-3xl font-semibold text-gray-900 dark:text-white">{resumeScoreState}%</p>
        </div>
        <div className="rounded-3xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-[#111827]">
          <p className="text-sm text-gray-500 dark:text-gray-400">Candidate Rank</p>
          <p className="mt-3 text-3xl font-semibold text-gray-900 dark:text-white">{candidateRankState}%</p>
        </div>
      </div>

      <div className="grid gap-6 lg:grid-cols-3 mb-6">
        <Card className="bg-white dark:bg-[#111827] border border-gray-200 dark:border-gray-700 rounded-3xl shadow-sm p-4">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              <ShieldCheck className="w-5 h-5 text-emerald-600" /> Resume Analysis
            </CardTitle>
            <CardDescription>Review your resume readiness and improvement opportunities.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 pt-2">
            <div className="rounded-3xl bg-slate-50 dark:bg-white/5 p-4">
              <p className="text-sm text-slate-500 dark:text-slate-400">Quality score</p>
              <p className="mt-2 text-4xl font-semibold text-slate-900 dark:text-white">{analysisScore}<span className="text-base text-slate-500 dark:text-slate-400">/100</span></p>
              <p className="mt-3 text-sm leading-6 text-slate-600 dark:text-slate-300">{analysisTakeaway}</p>
            </div>
        ...
        <Card className="bg-white dark:bg-[#111827] border border-gray-200 dark:border-gray-700 rounded-3xl shadow-sm p-4">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              <Activity className="w-5 h-5 text-sky-600" /> Mock Test Progress
            </CardTitle>
            <CardDescription>Track your latest mock test performance with quick guidance.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4 pt-2">
            {mockTests.map((test) => (
              <div key={test.id} className="space-y-3 rounded-3xl bg-slate-50 dark:bg-white/5 p-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-medium text-slate-900 dark:text-white">{test.title}</p>
                    <p className={`text-sm ${test.color}`}>{test.emoji} {test.status}</p>
                  </div>
                  <p className="text-xl font-semibold text-slate-900 dark:text-white">{test.score}%</p>
                </div>
                <Progress value={test.score} className="bg-slate-200 dark:bg-slate-800" />
              </div>
            ))}
          </CardContent>
        </Card>

        <Card className="bg-white dark:bg-[#111827] border border-gray-200 dark:border-gray-700 rounded-3xl shadow-sm p-4 flex flex-col">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-lg">
              <MessageSquare className="w-5 h-5 text-violet-600" /> AI Chatbot
            </CardTitle>
            <CardDescription>Ask quick questions about resume, tests, and career planning.</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col flex-1 gap-4 pt-2">
            <div className="min-h-[220px] max-h-[260px] overflow-y-auto rounded-3xl bg-slate-50 dark:bg-white/5 p-4 space-y-3">
              {chatMessages.map((message) => (
                <div key={message.id} className={`rounded-2xl p-3 ${message.role === "assistant" ? "bg-white dark:bg-gray-900" : "bg-slate-100 dark:bg-slate-800"}`}>
                  <p className="text-xs uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">{message.role}</p>
                  <p className="mt-1 text-sm text-slate-800 dark:text-slate-200">{message.text}</p>
                </div>
              ))}
            </div>
            <div className="flex items-center gap-2">
              ...
              />
              <Button onClick={sendChatMessage} className="whitespace-nowrap">
        ...
        <DialogContent className={`${getColumnColor(activeColumn).bg} ${getColumnColor(activeColumn).dark} border ${getColumnColor(activeColumn).border} ${getColumnColor(activeColumn).darkBorder} rounded-2xl shadow-lg`}>
          ...
          <DialogFooter className="flex justify-end space-x-2 mt-4">
            ...
            <Button
              ...
              className={`${newItemContent.trim() === "" ? "opacity-50 cursor-not-allowed" : ""} text-white ${activeColumn === 0 ? "bg-blue-600 hover:bg-blue-500" : activeColumn === 1 ? "bg-yellow-600 hover:bg-yellow-500" : activeColumn === 2 ? "bg-pink-600 hover:bg-pink-500" : "bg-emerald-600 hover:bg-emerald-500"}`}
            >
              Add Task
            </Button>
          </DialogFooter>
